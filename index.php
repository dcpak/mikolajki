...
const ALERT_ERROR_NUMBER_FLOAT = '<div class="error">Nijaki taki ten numer.</div>';
const ALERT_ERROR_NUMBER_RANGE = '<div class="error">Numer taki trochę nieszczególny.</div>';
const ALERT_ERROR_PASSWORD = '<div class="error">Numer dobry, tylko hasło niewydarzone.</div>';
    ...
    if($numberUser > 0 && $numberUser <= NUMBER_OF_PEOPLE) {
      ...
      if(file_exists(FILE_NUMBERS)) {
        ...
        foreach($lines as $line) {
          ...
          if(password_verify($numberUser, $line[0])) {
            $numberExists = true;
      ...
      if(!$numberExists) {
        ...
        for($i = 1; $i <= NUMBER_OF_PEOPLE; $i++)
          if(!in_array($i, $numbersTaken) && $i != $numberUser) {
            $allowed = true;
            foreach($numbersMatches as $key => $value)
              if(password_verify($i, $value))
                $allowed = false;
            if($allowed)
              $numbersAvailable[] = $i;
          }
        if(!count($numbersAvailable))
          for($i = 1; $i <= NUMBER_OF_PEOPLE; $i++)
            if(!in_array($i, $numbersTaken) && $i != $numberUser)
              $numbersAvailable[] = $i;
